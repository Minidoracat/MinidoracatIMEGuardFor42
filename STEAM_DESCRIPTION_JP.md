...
[h2]🌐 対応 IME と言語[/h2]
[list]
[*] [b]保護対象の IME：[/b]中国語（注音・倉頡・嘸蝦米・Microsoft／搜狗拼音・RIME…）、日本語（Microsoft IME・Google 日本語入力…）、韓国語——この 3 つはプレイヤーの実証あり。Windows 内蔵のベトナム語 Telex／VNI、インド系 Phonetic IME も仕組みは同じですが、ゲーム内では未検証です。
...
[list]
[*] 約 450 KB の単一 exe。.NET や Java などのランタイム不要、ダウンロードして実行するだけ。
